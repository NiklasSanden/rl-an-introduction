...
def call_algorithms(algo, run, cumulative_reward):
    ...
    while time_step < TIME_STEPS:
        if algo == 0:
            Q, model, model_keys, tau, time_step = dyna_Q_plus(ENVIRONMENT, AGENT, GAMMA, max_episodes=1, n=N, kappa=KAPPA, alpha=ALPHA, Q=Q, 
            ...
            Q, model, model_keys = dyna_Q(ENVIRONMENT, AGENT, GAMMA, max_episodes=1, n=N, alpha=ALPHA, Q=Q, model=model, model_keys=model_keys, log=False)
